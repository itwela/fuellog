...
export function TopExercisesList({ items, accent }: { items: TopExerciseItem[]; accent: string }) {
  return (
    <div className="rounded-2xl p-4" style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.06)" }}>
      <p className="text-xs font-semibold text-[#6a6a6a] mb-3">Most trained this week</p>
          ...
          {items.map((item, i) => (
            <div key={item.id} className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold tabular-nums shrink-0" style={{ color: accent, letterSpacing: "-0.02em" }}>
                {item.setsCompleted} set{item.setsCompleted !== 1 ? "s" : ""}
              </span>
            </div>
          ))}
        </div>
